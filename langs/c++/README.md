...
처음 출시 당시에는 C코드를 생성하는 생성기와 C컴파일러로 컴파일되는 구조여서, 100% C와 호환되는 구조였다. 그러나 이후에는 C++ 컴파일러가 개발되어서 지금은 대부분 C와 호환되지만 100%는 아니다. 다만 `const` 같이 C++의 기능이 C로 역수입된 경우도 있고, 서로 호환성을 어느 정도 유지하면서 발전하는 중이라고 생각하면 될듯 하다.
